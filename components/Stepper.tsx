import React, { useState, useEffect, Children, useRef, useLayoutEffect, type ReactNode } from 'react';
    ...
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: 'var(--maroon)', color: 'var(--manjal)' },
          complete: { scale: 1, backgroundColor: 'var(--manjal)', color: 'var(--maroon)' }
        }}
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
  const lineVariants = {
    incomplete: { width: 0, backgroundColor: 'rgba(255,202,0,0.15)' },
    complete: { width: '100%', backgroundColor: 'var(--manjal)' }
  };
